...
import {MyEvent} from "@/features/event/types/MyEvent";
      ...
      <ul className={s.cards}>
        {
          events.map((card, i) => {
            ...
            if (i === 1) bgClass = s.green
            return <li key={i} className={`${s.card} ${bgClass}`}>

              {
                card.badge && <Badge height={20} className={s.badge}>{card.badge}</Badge>
              }

              <div className={s.period}>{card.date_text}</div>
